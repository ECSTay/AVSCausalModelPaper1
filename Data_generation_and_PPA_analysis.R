...
sim_fun <- function(){
  dat_ref <- newdatfunction(N_ref, theta = 0.3, epsilon = 2/3, eta = 0.1, tau_sp = 1.5, mu_sp = 1.35, phi = 0.01, tau_ma = 3, mu_ma = 5)
  ...
  drp <- utils::capture.output(fit <- PPAmod$sample(data = old_data, chains = 4))#1000 warm-up, #2000 iter
  
  postr <- as_draws_matrix(fit$draws(variables = c("p")))
  betas <- as_draws_matrix(fit$draws(variables = c("beta")))
  
  diag <- fit$print(c("alpha","beta"))
...
out_LLHH <- rbindlist(mclapply(1:N_sim, function(i) sim_fun(N_ref = 50000, N_new = 4000, tau_sp = 1.8, tau_ma = 4), mc.cores = 20), idcol = "sim")
out_LMHH <- rbindlist(mclapply(1:N_sim, function(i) sim_fun(N_ref = 50000, N_new = 4000, eta = 0.3, tau_sp = 1.8, tau_ma = 5), mc.cores = 20), idcol = "sim")
out_LHHH <- rbindlist(mclapply(1:N_sim, function(i) sim_fun(N_ref = 50000, N_new = 4000, eta = 0.4, tau_sp = 1.8, tau_ma = 5), mc.cores = 20), idcol = "sim")
...
out_HMHH <- rbindlist(mclapply(1:N_sim, function(i) sim_fun(N_ref = 50000, N_new = 4000, theta = 0.6, eta = 0.3, tau_sp = 1.8, tau_ma = 5), mc.cores = 20), idcol = "sim")
out_HHHH <- rbindlist(mclapply(1:N_sim, function(i) sim_fun(N_ref = 50000, N_new = 4000, theta = 0.6, eta = 0.4, tau_sp = 1.8, tau_ma = 5), mc.cores = 20), idcol = "sim")
